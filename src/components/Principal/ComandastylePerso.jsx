...
const ComandaStylePerso = ({ onGoToMenu, initialOptions }) => {
  ...
  const { permitirGuarnicionesSeparadas, deshabilitarAgrupacionGuarniciones, deshabilitarOrdenSecuencialGuarniciones, tiemposGuarnicion, primerToqueFinalizarAsignado, entregarPlatoEnteroAbsoluto, ocultarAnularEnTablasKds } = useConfiguracionCocina(getToken);
  ...
  const determinarAccionBoton = useCallback(() => {
    ...
    // CASO 2: Platos en estado 'dejar' (rojo) -> CAMBIAR_PLATO
  ...
  return (
    <div className={`w-full ${isFullscreen ? 'h-screen' : 'min-h-screen'} flex flex-col ${bgMain} ${textMain} overflow-hidden`}>
      ...
      {/* Dejar comanda / auditoría (platos usan CAMBIAR PLATO) */}
      <DejarPlatoModal
