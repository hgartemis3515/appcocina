export const CLAVES_CONTADOR_DEFAULT = ['arroz', 'papa frita', 'ensalada'];
export const ETIQUETAS_CONTADOR_DEFAULT = {
  arroz: 'Arroz',
  'papa frita': 'Papa frita',
  ensalada: 'Ensalada',
};
export const MAX_CLAVES_CONTADOR = 3;

export function extraerNombreOpcion(op) {
  if (op == null) return '';
  if (typeof op === 'string') return op.trim();
  return String(op.nombre || op.opcion || '').trim();
}

export function claveNombreComplemento(nombre) {
...
export function sugerirNombreCanonico(variantes) {
  const map = new Map();
...
export function normalizarClavesContador(claves) {
  let list = claves;
...
/**
 * @param {Array<{ nombre: string, cantidad?: number, pronombre?: string }>} filas
 * @param {{ conPronombre?: boolean }} opts
 */
export function contarGuarnicionesPorNombre(filas, opts = {}) {
...
export function textoContadorGuarniciones(filas) {
